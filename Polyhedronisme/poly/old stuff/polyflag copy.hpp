...
public:
  ...
  Polyhedron topoly(string name = "unknown polyhedron") {
    ...
    for (auto &flag : flags) {
      ...
      v = vtx[v];                         //  goto next vertex

      for (int faceCTR = 0; v != v0; faceCTR++) { // loop until back to start
        faces[ctr].push_back(vertidxs[v]);
        v = vtx[v];

        // necessary during development to prevent browser hangs on badly formed
        // flagsets
        //        if (faceCTR > MAX_FACE_SIDEDNESS) {
        //          printf("Bad flag spec, have a neverending face: %d\n", ctr);
        //          break;
        //        }
      }
      ctr++;
